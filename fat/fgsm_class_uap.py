...
class FGSM_Class_UAP(ATBase):

    def __init__(self, model, eps=8.0 / 255, log_dir='./log_cifar10/', name='fgsm_class_uap', device=None, seed=0,
                 uap_eps=10 / 255.0):
        super(FGSM_Class_UAP, self).__init__(model, eps=eps, log_dir=log_dir, name=name, device=device, seed=seed)
    ...
    def train(self, opt, scheduler, train_loader, test_loader, total_epoch=110, label_smoothing=0.4,
              weight_average=True, tau=0.9995, class_num=10, image_shape=(3, 32, 32), eval_start=90):
        ...
        loss_fn = MSELoss()
        uaps = torch.zeros((class_num, *image_shape)).uniform_(-self.uap_eps, self.uap_eps).to(self.device)
        uaps = torch.clamp(self.uap_eps * torch.sign(uaps), -self.uap_eps, self.uap_eps)

        momentum = torch.zeros((class_num, *image_shape)).to(self.device)
        ...
        for epoch in range(total_epoch):
            ...
            for images, labels in train_loader:
                ...
                uap_noise = uaps[labels].clone()
                adv_images = images.clone() + uap_noise
